#!/usr/bin/env bash
# e2e/run-draw.sh — THE single authoritative DRAWING e2e entry script (milestone
# D4). It consolidates the D2 (control) + D3 (display) flow into one hermetic
# script that proves, end-to-end, the full drawing round-trip:
#
#   setup  → boot BOTH x86_64 emulators (B=target running the DRAW app,
#            A=controller running our android-app), build native + APKs OFFLINE.
#   connect→ bridge B's adbd onto 10.0.2.2:6555, drive A's connect flow until a
#            stable scrcpy stream (server on B + 'Connected' on A), derive the
#            A->B transform from 2 calibration taps.
#   CONTROL→ inject strokes on A and assert each is RECEIVED by B's draw app as a
#            correct multi-point drag (start/effEnd within tol, points>2).
#   DISPLAY→ screenshot A before/after each stroke; assert the round-tripped red
#            line appears on A's decoded remote view (before~0, after>>threshold),
#            cross-validated by location (a B->A display affine fitted from the
#            single diagonal stroke predicts where the L's arms must land on A).
#   L shape→ two chained swipes forming an L: assert BOTH segments on B (geometry)
#            AND red along both predicted arms on A (visibility).
#
# Exit 0 ONLY if ALL pass (connection + single-stroke B-geometry + A round-trip +
# the L on both B and A). Self-cleaning trap. Inner runs --network none by default
# (hermetic; every build input baked into scrcpy-e2e:dev). E2E_ALLOW_NET=1 is the
# debug-only escape hatch. Artifacts (A before/after + L before/after + B
# screencaps, detect_red outputs, the per-stroke/per-shape evidence table) go to
# the mounted /artifacts dir.
#
# Structure MIRRORS e2e/run.sh (the tap e2e): an OUTER mode (build image if
# missing; docker-run ITSELF --inner with /dev/kvm + repo + gradle-cache +
# artifacts mounts; SUCCESS/FAILURE banner; propagate exit code) and an INNER mode
# (--inner) that runs the full drawing flow. It SOURCES e2e/run.sh for the proven
# boot/build/connect helpers (run.sh's dispatch is BASH_SOURCE-guarded so sourcing
# only exposes helpers) + lib-draw/lib-automation/lib-net — NO copy-pasted logic.
#
...
# Source run.sh for ALL its shared config + INNER helpers (it will NOT dispatch
# because it is sourced, not executed): log/fail, EMU_MEM/BOOT_TIMEOUT/STREAM_SECS,
# the WORKSPACE/ARTIFACTS/APP_DIR/TARGET_DIR/APK/NATIVE_OUT vars, the AVD/serial/
# port constants, cleanup/create_avd/launch_emu/wait_boot/boot_both,
...
draw_outer() {
  ...
  local artifacts="${SCRCPY_E2E_DRAW_ARTIFACTS:-$REPO_ROOT/e2e/artifacts}"
  mkdir -p "$artifacts"
  rm -rf "${artifacts:?}/"* 2>/dev/null || true

  log "DRAW e2e OUTER: image=$IMAGE repo=$REPO_ROOT artifacts=$artifacts"
  ...
  # Hermetic by DEFAULT: the inner container gets NO network. Every build input is
  # baked into the image (apt tools, Android SDK/NDK, native source, the offline
  # GRADLE_USER_HOME=/opt/gradle-home + Pillow). E2E_ALLOW_NET=1 is debug-only.
  ...
  log "Launching DRAW e2e INNER container (docker run --device /dev/kvm $netarg)"
  set +e
  docker run --rm \
    $netarg \
    --device /dev/kvm \
    ...
    "$IMAGE" \
    bash /workspace/e2e/run-draw.sh --inner
  local rc=$?
  set -e
  if [ "$rc" -eq 0 ]; then
    log "==================== DRAW e2e SUCCESS (drawing round-trip green) ===================="
  else
    log "==================== DRAW e2e FAILURE (rc=$rc) ===================="
  fi
  return "$rc"
}

# ---------------------------------------------------------------------------
# INNER MODE — boot both, connect A->B(draw), assert strokes A->B AND visible-on-A.
# ---------------------------------------------------------------------------
...
#   start/effEnd within tol of the expected B endpoints. Returns 0 PASS / 1 FAIL
#   and sets globals B_VERDICT, B_NCUR, B_RECORDED, B_DETAIL for the caller.
...
inner_draw() {
  ...
  log "INNER(draw): full drawing e2e on $(uname -m); A target = 10.0.2.2:${BRIDGE_PORT}; B-tol=max(${AB_ABS_TOL}px,${AB_REL_TOL_PCT}%)"
  ...
  # ==========================================================================
  # A's REMOTE-VIEW CROP. A keeps its status bar (top) + nav bar (bottom) and
  # renders B's video (magnified) into A's content rect. We restrict detect_red on
  # A to a box that excludes:
  ...
  local EVID="$ARTIFACTS/drawing-e2e-evidence.txt"
  {
    echo "=== DRAWING e2e: stroke-on-A -> received-by-B (control) + displayed-back-on-A (display) evidence ($(date '+%F %T')) ==="
  ...
  # ==========================================================================
  # PART 1 — SINGLE DIAGONAL STROKE: CONTROL (B-geometry: the stroke on A is
  # received by B as a correct multi-point drag) + DISPLAY round-trip (before~0,
  ...
  # Assert B received it (geometry) — this is the CONTROL proof for PART 1.
  ...
  prev_strokes="$B_NCUR"
  log "  PART1 B stroke (CONTROL): [$P1_B_VERDICT] $P1_B_DETAIL"
  ...
  {
    echo "--- PART 1: SINGLE DIAGONAL STROKE (control geometry on B + display round-trip on A) ---"
    echo "A swipe : ($SAX0,$SAY0) -> ($SAX1,$SAY1)"
    echo "B geometry [$P1_B_VERDICT] (CONTROL: stroke on A received by B): $P1_B_DETAIL"
    ...
    echo "PART1 verdict: B(control)=$P1_B_VERDICT  A(display)=$P1_A_VERDICT"
    echo
  } >>"$EVID"
  ...
  # ==========================================================================
  # PART 2 — MULTI-SEGMENT L. Two chained swipes forming an L in A's UPPER region
  # (so the whole L is visible on A, unclipped):
  ...
  {
    echo "--- PART 2: MULTI-SEGMENT L (control geometry on B + display round-trip on A) ---"
    ...
    echo "L verdict: B(control)=$L_B_VERDICT  A(display)=$L_A_VERDICT"
    echo
  } >>"$EVID"
  ...
  [ "$all_pass" -eq 1 ] \
    || fail "drawing round-trip FAILED — see $EVID (single stroke B/A and/or L B/A did not pass)"
  log "PASS: each stroke drawn on A was RECEIVED by B (control geometry) AND DISPLAYED back red on A (display round-trip); the L is visible along both segments on A and recorded as 2 strokes on B."
  exit 0
}
...
draw_inner() {
  mkdir -p "$ARTIFACTS"
  trap cleanup EXIT
  log "DRAW e2e INNER: $(uname -m); whoami=$(whoami); EMU_MEM=${EMU_MEM} BOOT_TIMEOUT=${BOOT_TIMEOUT} STREAM_SECS=${STREAM_SECS}"
